#!/usr/bin/env bash
# Sync the static public site (public-site/) to S3. Run after edits to the
...
echo "→ syncing public-site/ to s3://$TP_BUCKET/  (profile: $AWS_PROFILE)"

# HTML/JS/CSS — long-ish browser cache; admin app will only change these when
# the codebase is updated.
aws s3 sync public-site/ "s3://$TP_BUCKET/" \
  --exclude "data/*" \
